extends Node2D
class_name NoteNode
## Single chart note — tap, hold, or lumen arc.

enum NoteType { TAP, HOLD, ARC }
...
func _draw() -> void:
	...
	if note_type == NoteType.HOLD:
		...
		base_col = Color(0.85, 0.35, 1.0, 0.95)
	var glow_col := base_col.lightened(0.35 + _glow * 0.4)
	draw_circle(Vector2.ZERO, _radius + 6.0 * _glow, glow_col * Color(1, 1, 1, 0.25))
